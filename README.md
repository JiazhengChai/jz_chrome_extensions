...
- `page-to-markdown`: extracts readable page text and saves it as Markdown or through a PDF export flow that lands in the same extension folder.
...
- `storage` on `page-to-markdown`: pass generated content into its PDF export tab.
- `debugger` on `page-to-markdown`: print the preview tab directly to PDF so the downloaded file can reuse the Markdown folder path.
...
- The page-to-markdown popup can also open a dedicated PDF preview and download the rendered file into the same folder used for Markdown.
